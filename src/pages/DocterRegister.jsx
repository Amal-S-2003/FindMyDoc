...
import { docterRegister } from "../services/allAPIS";
...
function DoctorRegister() {
  ...
  const [formData, setFormData] = useState({
    ...
    specializations: [],
    qualifications: [],
    experiences: [{ place: "", from: "", to: "" }], // Changed 'years' to 'from' and 'to'
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      setFormData((prev) => ({ ...prev, profilePicFile: file })); // Store the actual file for submission
    }
  };
  ...
  const validateStep2 = () => {
    ...
    if (formData.experiences.some((exp) => !exp.place || !exp.from || !exp.to)) {
      newErrors.experiences = "All experience fields are required";
      isValid = false;
    }
    if (formData.certificates.some((cert) => !cert.description && cert.file)) {
      newErrors.certificates = formData.certificates.map((cert) =>
        !cert.description && cert.file ? "Description is required" : ""
      );
      isValid = false;
    }
    if (formData.awards.some((award) => !award.description && award.file)) {
      newErrors.awards = formData.awards.map((award) =>
        !award.description && award.file ? "Description is required" : ""
      );
      isValid = false;
    }
  ...
  const handleDeleteAward = (index) => {
    handleInputChange(
      "awards",
      formData.awards.filter((_, i) => i !== index)
    );
  };
  ...
  const handleRequestForRegister = async (e) => {
    e.preventDefault();
    if (validateStep3()) {
      setLoading(true);
      ...
      for (const key in formData) {
        if (key === "specializations" || key === "qualifications") {
          formDataToSend.append(key, JSON.stringify(formData[key].map(item => item.value)));
          ...
          formData.certificates.forEach((cert) => {
            if (cert.file) {
              ...
              formDataToSend.append("certificateDescriptions", cert.description);
            }
          });
        } else if (key === "awards") {
          formData.awards.forEach((award) => {
            if (award.file) {
              formDataToSend.append("awards", award.file);
              formDataToSend.append("awardDescriptions", award.description);
            }
          });
        } else if (key === "availability") {
          formDataToSend.append(key, JSON.stringify(formData[key]));
        } else if (key === "profilePicFile" && formData[key]) {
          formDataToSend.append("profilePic", formData[key]);
        } else if (key !== "profilePic" && key !== "profilePicFile") {
    ...
    const reqHeader = {
      "Content-Type": "multipart/form-data",
        ...
        const result = await docterRegister(formDataToSend,reqHeader);
      ...
      <div className="p-6 w-full max-w-2xl bg-white rounded-lg shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:col-span-2">
                ...
                <div className="relative">
                  ...
                  <input
                    type="date"
                    className={`pl-10 pt-6 p-2 border ${
          ...
          >
            <div className="relative mb-4">
              ...
              <input
                ...
                className={`w-full pl-12 py-3 border ${
                  errors.medicalRegNumber ? "border-red-500" : "border-gray-300"
                } rounded-lg focus:ring-2 focus:ring-red-400 focus:outline-none`}
                value={formData.medicalRegNumber}
                onChange={(e) =>
                  handleInputChange("medicalRegNumber", e.target.value)
                }
              />
            ...
            <div className="mb-4">
              ...
              <Select
                ...
                onChange={(selectedOptions) =>
                  handleInputChange("specializations", selectedOptions)
                }
                className={
                  errors.specializations ? "border border-red-500 rounded" : ""
                }
              />
            ...
            <div className="mb-4">
              ...
              <Select
                ...
                onChange={(selectedOptions) =>
                  handleInputChange("qualifications", selectedOptions)
                }
                className={
                  errors.qualifications ? "border border-red-500 rounded" : ""
                }
              />
              ...
              {formData.experiences.map((exp, index) => (
                ...
                >
                  <input
                    ...
                    className={`w-full md:w-1/2 py-3 px-4 border ${
                  ...
                  <div className="flex gap-2 w-full md:w-1/2">
                    <input
                      ...
                      className={`w-1/2 py-3 px-4 border ${
                        errors.experiences
                          ? "border-red-500"
                          : "border-gray-300"
                      } rounded-lg focus:ring-2 focus:ring-red-400 focus:outline-none`}
                ...
                <div key={index} className="flex items-center gap-4 mb-3">
                  ...
                  <input
                    ...
                    className={`w-full py-2 px-4 border ${
                      errors.certificates && errors.certificates[index]
                        ? "border-red-500"
                        : "border-gray-300"
                    } rounded-lg focus:ring-2 focus:ring-red-400 focus:outline-none`}
              ...
              {formData.awards.map((award, index) => (
                <div key={index} className="flex items-center gap-4 mb-3">
                  ...
                  <input
                    ...
                    className={`w-full py-2 px-4 border ${
                      errors.awards && errors.awards[index]
                        ? "border-red-500"
                        : "border-gray-300"
                    } rounded-lg focus:ring-2 focus:ring-red-400 focus:outline-none`}
          ...
          >
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <input
                        ...
                        disabled={formData.availability[day].notAvailable}
                    ...
                    <div className="flex items-center mt-2">
                      <input
                        ...
                        id={`notAvailable-${day}`}
                        className="mr-2"
                        checked={formData.availability[day].notAvailable}
                        onChange={(e) =>
            ...
            >
              ...
              <input
                ...
                } rounded w-full bg-gray-50 focus:ring-2 focus:ring-red-400 focus:outline-none`}
        ...
        <div className="flex justify-between mt-6">
          {step > 1 && (
            <button
              onClick={handlePrev}
              className="px-4 py-2 bg-gray-400 text-white cursor-pointer rounded hover:bg-gray-500 transition"
            >
              Back
            </button>
          )}
          {step < 3 ? (
            <button
              onClick={handleNext}
              className="px-4 py-2 font-medium bg-red-600 text-white rounded cursor-pointer hover:bg-red-700 transition ml-auto"
            >
              Next
            </button>
          ) : (
            <button
              onClick={(e)=>handleRequestForRegister(e)}
              className="px-4 py-2 bg-red-600 text-white rounded cursor-pointer hover:bg-red-700 transition ml-auto"
            >
